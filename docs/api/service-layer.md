...
確認と書き込みの間に別のリクエストが入ると通り抜ける。同時実行を実際に
止められるのは DB の一意制約だけなので、両方を置く。

- DB に一意制約を置く（一覧は [ER 図](../database/er-diagram.md)）
- Service Layer は違反（`23505`）を拾い、**事前チェックと同じ文言**の
  失敗 Result に変換する（`isUniqueViolation()` / `src/services/db-errors.ts`）
- 一意制約違反**以外**の DB エラーは握り潰さず再送出する。
  接続断まで「重複しています」と表示すると原因を見失う
...
| 処理                          | 中途半端に終わるとどうなるか                          |
| ----------------------------- | ----------------------------------------------------- |
| `createOrganizationWithOwner` | 誰も入れない組織が残る（削除は service_role 専用）    |
| `acceptInvitation`            | メンバーなのに招待が未消費 / 従業員レコードと未紐付け |
