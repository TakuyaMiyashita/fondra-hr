...
describe('EvaluationsTab', () => {
  ...
  it('取得が失敗したら空状態ではなく失敗として出す', async () => {
    ...
    expect(await screen.findByText('評価記録を取得できませんでした')).toBeInTheDocument();
    expect(screen.queryByText('評価記録がありません')).not.toBeInTheDocument();
  });
  ...
  it('再試行で取得し直す', async () => {
    ...
    renderWithQuery(<EvaluationsTab employeeId={EMPLOYEE_ID} />);
    await screen.findByText('評価記録を取得できませんでした');

    const before = fetchEmployeeEvaluations.mock.calls.length;
    await userEvent.click(screen.getByRole('button', { name: '再試行' }));

    await waitFor(() => expect(fetchEmployeeEvaluations.mock.calls.length).toBeGreaterThan(before));
  });
